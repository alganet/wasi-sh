...
// ─── compgen ─────────────────────────────────────────────────────────────────
// The completion engine, reachable without the guest's line editor — see
// build/ash-compgen.patch for why that is a separate thing from Tab at a
// prompt. These need no terminal and no ring, which is the point: the consumer
// is a terminal that edits its lines somewhere else.

test('compgen -c completes a command name against the applet table', async () => {
  const r = await sh('compgen -c ec\n');
  assert.equal(r.stdout, 'echo\n');
  assert.equal(r.stderr, '');
});

test('compgen -c deduplicates a name that is both an applet and a builtin', async () => {
  // `echo` is both, so the raw engine offers it twice. input_tab() has always
  // deduplicated before showing anything; the hoist in ash-compgen.patch is
  // what makes this path agree with it.
  const r = await sh('compgen -c echo\n');
  assert.equal(r.stdout, 'echo\n', 'exactly once');
});

test('compgen -c lists shell functions and aliases, which no applet table has', async () => {
  const r = await sh('myfunc() { :; }\nalias myalias=ls\ncompgen -c my\n');
  assert.deepEqual(r.stdout.split('\n').filter(Boolean).sort(), ['myalias', 'myfunc']);
});

test('compgen -f answers BASENAMES, not paths', async () => {
  // Upstream's shape, kept deliberately: the caller re-attaches the directory
  // it asked under. Changing it here would make a host's Tab and the guest's
  // Tab produce different text for the same word.
  const r = await sh('compgen -f sub/ne\n', { files: { '/sub/nested.txt': 'x', '/sub/other.txt': 'y' } });
  assert.equal(r.stdout, 'nested.txt\n', 'not "sub/nested.txt"');
});

test('compgen -f marks a directory with a trailing slash and a file without', async () => {
  const r = await sh('compgen -f mark\n', { files: { '/markfile.txt': 'x', '/markdir/inner': 'y' } });
  assert.deepEqual(r.stdout.split('\n').filter(Boolean).sort(), ['markdir/', 'markfile.txt']);
});

test('compgen -d offers directories only', async () => {
  const r = await sh('compgen -d only\n', { files: { '/onlyfile.txt': 'x', '/onlydir/inner': 'y' } });
  assert.equal(r.stdout, 'onlydir/\n');
});

test('compgen reports status 1 when nothing matches, and prints nothing', async () => {
  const r = await sh('compgen -c zzz_no_such_command\necho "rc=$?"\n');
  assert.equal(r.stdout, 'rc=1\n');
});

test('compgen -c follows a PATH assigned after the shell started', async () => {
  // path_lookup is refreshed from pathval() on every call, because a `cd` and a
  // PATH= can both land between two completions in a live session.
  const r = await sh('mkdir -p /opt/bin\n: > /opt/bin/mytool_xyz\nchmod +x /opt/bin/mytool_xyz\n'
    + 'PATH=/opt/bin compgen -c mytool_\n');
  assert.equal(r.stdout, 'mytool_xyz\n');
});

test('compgen rejects an unknown kind rather than guessing one', async () => {
  const r = await sh('compgen -x foo\n');
  assert.equal(r.stdout, '');
  assert.match(r.stderr, /illegal option -x/);
});

test('compgen -- lets a WORD start with a dash', async () => {
  // `ls -l<TAB>` asks for exactly this. Without the "--" the option parser ate
  // the word and completed the separator instead.
  const r = await sh('compgen -f -- -da\n', { files: { '/-dashed.txt': 'x', '/other.txt': 'y' } });
  assert.equal(r.stdout, '-dashed.txt\n');
});

test('kill survived job control being compiled out', async () => {
  // `kill` sits inside busybox's `#if JOBS` block, so turning ASH_JOB_CONTROL
  // off to keep the nested-shell ttyfd invariant took it too. It is back as
  // the applet (CONFIG_KILL=y), which is the more honest of the two here:
  // there are no jobs for a `%1` spec to name, and `kill -l` is real work that
  // needs no processes at all.
  const r = await sh('kill -l 9\n');
  assert.equal(r.stdout, 'KILL\n');
  assert.equal(r.stderr, '');
});

test('and jobs/fg/bg really are gone, as the docs say', async () => {
